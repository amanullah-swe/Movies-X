...
export const fetchRecomendationMoviesAsync = createAsyncThunk(
  ...
  async ({ toogle, movieId, genres }) => {
    const response = await fetchRecomendationMovies({
      toogle,
      movieId,
      genres,
    });
...
export const {
  setTendingToogle,
  setTrendingRenderCount,
  setPopularToogle,
  setPopularRenderCount,
  setTopRatedToogle,
  setTopRatedRenderCount,
} = movie.actions;

export const selectMovieDetial = (state) => state.movie.movieDetail;
export const selectMovieCredits = (state) => state.movie.credits;
export const selectMovieRelatedVideos = (state) =>
  state.movie.movieRelatedVideos;
export const selectSimilarMovies = (state) => state.movie.similarMovies;
export const selectRecomendationMovies = (state) =>
  state.movie.recomendationMovies;
export const isMovieDetailLoading = (state) => state.status;
